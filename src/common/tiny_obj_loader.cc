...
exportFaceGroupToShape(
  ResourceMesh::shape_t& shape,
  const std::vector<float> &in_positions,
  const std::vector<float> &in_normals,
  const std::vector<float> &in_texcoords,
  const std::vector<std::vector<vertex_index> >& faceGroup,
  const ResourceMesh::material_t &material,
...
void InitMaterial(ResourceMesh::material_t& material) {
  material.name = "";
...
std::string LoadMtl (
  std::map<std::string, ResourceMesh::material_t>& material_map,
  const char* filename,
  const char* mtl_basepath)
{
  ...
  while (ifs.peek() != -1) {
	...
	if ((0 == strncmp(token, "newmtl", 6)) && isSpace((token[6]))) {
	  ...
	  material_map.insert(std::pair<std::string, ResourceMesh::material_t>(material.name, material));
	  ...
	  char namebuf[4096];
	  token += 7;
	  sscanf_s(token, "%s", namebuf, sizeof(namebuf));
...
std::string
LoadObj(
  std::vector<ResourceMesh::shape_t>& shapes,
  const char* filename,
  const char* mtl_basepath)
{
